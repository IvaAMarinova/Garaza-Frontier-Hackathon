...
export default function MindMap({ initialText, isDarkMode = false }) {
    const { nodes, draggingId, containerRef, connections, backgroundOffset, isPanningBackground, newlyCreatedNodes, updatedNodes, sessionId, isLoading, zoomLevel, goal, addNode, deleteNode, editNode, removeConnection, zoomIn, zoomOut, resetZoom, handleFinish, handleMouseDown, handleBackgroundMouseDown, } = useMindMap(initialText, isDarkMode);
    return (_jsxs("div", { ref: containerRef, className: `mind-map-canvas relative w-full h-screen overflow-hidden bg-gradient-to-br transition-colors duration-300 ${isDarkMode
            ? "from-slate-950 to-slate-900"
            : "from-slate-50 to-slate-100"} ${isPanningBackground ? "cursor-grabbing" : "cursor-grab"}`, onMouseDown: handleBackgroundMouseDown, role: "application", "aria-label": "Mind map canvas - drag nodes to move them or pan background", children: [isLoading && (_jsx("div", { className: "absolute inset-0 flex items-center justify-center bg-black/20 z-50", children: _jsx("div", { className: `rounded-lg p-6 shadow-lg ${isDarkMode ? "bg-slate-800" : "bg-white"}`, children: _jsxs("div", { className: "flex items-center space-x-3", children: [_jsx("div", { className: "animate-spin rounded-full h-6 w-6 border-b-2 border-blue-600" }), _jsx("span", { className: isDarkMode ? "text-slate-300" : "text-slate-700", children: "Loading tic tac toe concepts..." })] }) }) })), _jsx(GoalDisplay, { goal: goal, onFinish: handleFinish, isDarkMode: isDarkMode }), sessionId && !isLoading && (_jsxs("div", { className: `absolute top-4 left-4 rounded-lg px-3 py-2 text-xs backdrop-blur-sm ${isDarkMode
