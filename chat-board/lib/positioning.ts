...
export function estimateNodeDimensions(node: Node): { width: number; height: number } {
  ...
  const maxTextWidth = Math.max(
    ...textLines.map(line => line.length * CHAR_WIDTH_ESTIMATE),
    MIN_NODE_WIDTH - BASE_PADDING
  )
  ...
  if (node.content.codeBlock) {
    ...
    const maxCodeWidth = Math.max(
      ...codeLines.map(line => line.length * CHAR_WIDTH_ESTIMATE),
      200 // Minimum code block width
    )
 ...
 */
export function calculateAndStoreBounds(node: Node): Node {
  const { width, height } = estimateNodeDimensions(node)
  const halfWidth = width / 2
  const halfHeight = height / 2
 ...
 */
function doBoundsOverlap(bounds1: NodeBounds, bounds2: NodeBounds): boolean {
  // Check if boxes are separated by at least MIN_DISTANCE_BETWEEN_NODES
  // If not, they overlap or are too close
  return !(
    bounds1.topRight.x < bounds2.topLeft.x - MIN_DISTANCE_BETWEEN_NODES ||
 ...
 */
export function areNodesTooClose(bounds1: NodeBounds, bounds2: NodeBounds): boolean {
  ...
  const minRequiredDistance = 
    (Math.sqrt(bounds1.width ** 2 + bounds1.height ** 2) / 2) +
 ...
 */
function getNodeDirection(node: Node, referenceNode: Node): number {
 ...
 */
export function pushNodesAwayFromDragged(
  draggedNodeBounds: NodeBounds,
  allNodes: Node[],
  excludeNodeId: string
): Node[] {
  ...
  for (let i = 0; i < adjustedNodes.length; i++) {
    ...
    if (areNodesTooClose(draggedNodeBounds, nodeBounds)) {
      ...
      const requiredDistance = 
        (Math.sqrt(draggedNodeBounds.width ** 2 + draggedNodeBounds.height ** 2) / 2) +
      ...
      if (parentNode && node.parentId !== null) {
        ...
        if (!areNodesTooClose(draggedNodeBounds, testBounds)) {
          ...
          const safePosition = findSafePosition(
            ...
            width,
            height,
            adjustedNodes.filter(n => n.id !== node.id && n.id !== excludeNodeId).concat([{ 
              id: excludeNodeId, 
      ...
      if (distance === 0) {
        ...
        if (parentNode) {
          ...
          const safePosition = findSafePosition(
            ...
            width,
            height,
            adjustedNodes.filter(n => n.id !== node.id),
            getNodeDirection(node, parentNode)
          )
      ...
      if (areNodesTooClose(currentBounds, otherBounds)) {
        ...
        if (parentNode) {
          ...
          const safePosition = findSafePosition(
            ...
            width,
            height,
            fixedNodes.filter(n => n.id !== currentNode.id),
            getNodeDirection(currentNode, parentNode)
          )
          // Update node with new position and recalculate bounds
...
export function findOverlappingNodes(
  nodes: Node[]
): Array<{ node1: Node; node2: Node; overlap: boolean }> {
  const overlaps: Array<{ node1: Node; node2: Node; overlap: boolean }> = []
